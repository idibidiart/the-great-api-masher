...
Our proposal is to make UI development more agile by leveraging the “Typed I/O" paradigm of GraphQL, an open source spec with NodeJS, Java and other implementations, that allows us to specify the data flow and business domain logic outside the UI and in flexible, easy to change way. By keeping the data flow and business logic outside the UI, in a declaratively specified, dynamic aggregation/orchestration tier, we may keep the UI as a thin I/O layer, with dynamic behaviors localized within UI components and client side router solely for the purpose of enabling user interaction and navigation.
