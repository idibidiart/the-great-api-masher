...
- When multiple queries need to be serializable, e.g. multiple queries from an autocomplete text box where the responses could come back out-of-order with respect to the requests, and when some responses should be cancellable, GraphQL doesn't provide a way to manage that. When the query in question is resolved by a single Type Resolver (i.e. query returns a type made entirely of primitive types), we can create a timestamp when initiating the API call in the resolver and return that timestamp as part of the data (as a derived field in the query's return type.) This way if the client receives data with timestamp smaller than one it has received already it can ignore it.   

-  If the API response can be interpreted differently by different clients that's a problem. Deriving client-only state (presentation state) using GraphQL types (in the query output) eliminates that problem. In other words if client-only state (presentation state) needs to be "inferred" from API response, it should be done using derived fields in the GraphQL query's return type, where normally the client would have to infer presentation state (based on presence/absence of certain fields or other types of inference) This is a feature of GraphQL that allows us to patch API response to eliminate the need to infer presentation state in the client, which along with the ability to describe data-flow tasks declaratively, allows us to keep business rules out of the client.
